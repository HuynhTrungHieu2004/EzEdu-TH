...
export default function StudentStatisticsPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const attempt of attempts) {
      if (attempt.question_set_id && !latestByExam.has(attempt.question_set_id)) {
        latestByExam.set(attempt.question_set_id, attempt);
      }
    }
    return {
                    ...
                    <tbody>
                      {stats.latest.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td><button type="button" className="btn-secondary" disabled={!item.question_set_id} onClick={() => item.question_set_id && navigate(`/question-sets/${item.question_set_id}`)}>Xem lại</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
